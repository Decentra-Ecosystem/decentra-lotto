...
import { ETHERLAND_ADDRESS_MAIN_NET } from '../../models/meta-mask.dictionary';

@Component({
  selector: 'app-etherland',
  templateUrl: './etherland.component.html',
  styleUrls: ['./etherland.component.css']
})
export class EtherlandComponent implements OnInit {
  ...
  charityAddress = ETHERLAND_ADDRESS_MAIN_NET;
  ...
  async getSummary(){
    this.statsService.drawStatsSub.subscribe({
      next: async stats => {
        (await this.statsService.getSummaryStats(ETHERLAND_ADDRESS_MAIN_NET))
