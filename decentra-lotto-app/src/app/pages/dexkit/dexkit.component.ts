...
import { DEXKIT_ADDRESS_MAIN_NET } from '../../models/meta-mask.dictionary';

@Component({
  selector: 'app-dexkit',
  templateUrl: './dexkit.component.html',
  styleUrls: ['./dexkit.component.css']
})
export class DexkitComponent implements OnInit {
  ...
  charityAddress = DEXKIT_ADDRESS_MAIN_NET;
  ...
  async getSummary(){
    this.statsService.drawStatsSub.subscribe({
      next: async stats => {
        (await this.statsService.getSummaryStats(DEXKIT_ADDRESS_MAIN_NET))
