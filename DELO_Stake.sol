...
/**
 * DELO staking Pool. Deposit DELO earn DELO.
*/
...
contract DELOStake is Owned {
    ...
    function CLAIMREWARD() public {
        if(totalDividends > stakers[msg.sender].fromTotalDividend){
            ...
            require(IERC20(DELO).transfer(msg.sender,owing), "ERROR: error in sending reward from contract");
   ...
   function yourStakedDELO(address staker) external view returns(uint256 stakedDELO){
        return stakers[staker].stakedTokens;
    }
        
     function yourDELOBalance(address user) external view returns(uint256 DELOBalance){
        return IERC20(DELO).balanceOf(user);
    }
}
